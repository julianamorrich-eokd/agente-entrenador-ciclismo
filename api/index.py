...
# 👇 nuevo: fallback por defecto
DEFAULT_ATHLETE_ID = os.getenv("DEFAULT_ATHLETE_ID", "13999313")
...
# 👇 nueva función para normalizar el athlete_id
def _normalize_aid(aid: str | None) -> str:
    bads = {"", "0", "12345", "123456", None}
    return DEFAULT_ATHLETE_ID if aid in bads else str(aid)
...
async def get_athlete(request: Request, athlete_id: str | None = None):
    if not _bearer_ok(request):
        raise HTTPException(status_code=403, detail="Forbidden")
    aid = _normalize_aid(athlete_id)   # 👈 usa el normalizador
    access = await _ensure_token(aid)
    async with httpx.AsyncClient(timeout=20) as client:
...
async def latest_activities(request: Request, athlete_id: str | None = None, per_page: int = 30):
    if not _bearer_ok(request):
        raise HTTPException(status_code=403, detail="Forbidden")
    aid = _normalize_aid(athlete_id)   # 👈 usa el normalizador
    access = await _ensure_token(aid)
